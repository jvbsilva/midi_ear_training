...
chords = {
    ...
    'iv':  [65, 69, 72], # F minor chord: F4, A4, C5
    ...
    'vii': [71, 74, 77], # B diminished chord: B4, D5, F5
    'VII': [71, 75, 78]  # B major chord: B4, D#5, F#5
}


def generate_chord_progression(progression, filename, velocity=64, duration=480):
    mid = MidiFile()
    track = MidiTrack()
    ...
    for chord_symbol in progression:
        ...
        track.append(Message('note_off', note=chord[0], velocity=velocity, time=duration))
...
# Example usage: Generate a I-IV-V-I progression in C major
generate_chord_progression(['I', 'IV', 'V', 'I'], 'chord_progression.mid')
